...
with methodology:
    ...
    st.markdown("After some cleaning, the resulting dataframe is the following:")
    ...
    st.write(taxi_data.head())
    infog1 = Image.open("images/infogram1.jpg")
    st.image(infog1, use_column_width=False,width=1000)
    st.markdown("My code is structured in different documents, all of them in .py format.")
    st.markdown("main.py contains the streamlit page code and color_functions.py contains all the functions that render palettes and that will be used in my page.")
    ...
    st.markdown("2. render_color_palette - Gets the output from extract_colors and turns the codes into the RGB Color.")
    st.markdown("3. overlay_palette - Overlays the inputed image together with the palette image.")
    st.markdown("We can see how the study_image function works in the following example:")
    infog2 = Image.open("images/infogram2.jpg")
    st.image(infog2, use_column_width=False,width=1000)
